...
{
    /// <summary>
    /// Ukkonen O(n^2) algorithm with suffix Links, skip/count trick
    /// As described in the book by D. Gusfield, Algorithms on Strings, Trees and Sequences
    /// </summary>
    public class SuffixTreeUkkonenQuadratic : SuffixTree
    {
        #region Fields
        ...
        #region Constructor
        public SuffixTreeUkkonenQuadratic(string inputText)
        {
        ...
        private static Location Navigate(Node parent, int from, int to, string substring, string text, bool useSkipCount)
        {
            ...
            while (true)
            {
                ...
                if (useSkipCount)
                {
                    var skip = Math.Min(to - k, child.end - child.start);
                    m += skip;
                    k += skip;
                }
                else
        ...
        private class UkkonenBuilder
        {
            ...
            private Next ApplyRule(Location location, int j, int i)
            {
                ...
                if (child.start + location.offsetInEdge == child.end)
                {
                    if (IsInternal(child))
                    {
                        ...
                        {
                            ApplyRule3();
                            return new Next(location.parent, child.end - child.start, 3);
                        }
                        else
                        {
                            ApplyRule2(child, -1, 0, location.offsetInString, j, i + 1);
                            return new Next(location.parent, child.end - child.start, 2);
                        }
                    }
                    else
                    {
